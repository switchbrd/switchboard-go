...
function SwitchboardApi(im, url, username, password) {
	...
	self.submit_search_number = function(phone_number) {
		...
		/*p.add_callback(function (result) {
			return (typeof result == "undefined") ? null : result.in_cug;
		});*/
		return p;
	};  
    ...
    // Session 2
        ...
        self.add_state(new ChoiceState(
            ...
            [
                ...
                new Choice("2", _.gettext("Update Registration Number"))
            ]
        ));               
		
  		
  		self.add_state(new FreeText(
			"enter_number_to_check_1",
          	"search_number_in_cug",
			_.gettext("Please enter a number in the format 07XXXXXXXX")
        ));
  
		self.add_state(new EndState(
			"invalid_action_selection",          	
			_.gettext("Please enter a number!"),
          	"check_cug_number"
		));               
